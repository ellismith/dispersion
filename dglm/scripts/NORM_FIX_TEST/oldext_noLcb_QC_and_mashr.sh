...
set -e
CT=$1
SRC=/scratch/easmit31/dispersion/dglm/disp_age__FIXEDDISP2_300c/${CT}/dglm_checkpoints_cutoff0.5
DEST=/scratch/easmit31/dispersion/dglm/disp_age__FIXEDDISP2_300c_noLcb_QCfiltered/${CT}/dglm_checkpoints_cutoff0.5
mkdir -p $DEST
...
for (f in files) {
  r = readRDS(f)
  arr = r\$array
  if ('lCb' %in% dimnames(arr)[[3]]) {
    keep_regions = setdiff(dimnames(arr)[[3]], 'lCb')
    arr = arr[, , keep_regions, drop=FALSE]
  }
...
cat('$CT (old ext, noLcb, QC) flagged:', n_flagged_total, '\n')
"
