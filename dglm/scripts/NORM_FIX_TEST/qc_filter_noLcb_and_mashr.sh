...
set -e
CT=$1
SRC=/scratch/easmit31/dispersion/dglm/disp_age__FINAL_autosomeX_300c_noLcb/${CT}/dglm_checkpoints_cutoff0.5
DEST=/scratch/easmit31/dispersion/dglm/disp_age__FINAL_autosomeX_300c_noLcb_QCfiltered/${CT}/dglm_checkpoints_cutoff0.5
mkdir -p $DEST
...
for (f in files) {
  ...
  if (!is.null(bad) && nrow(bad) > 0) {
    for (i in seq_len(nrow(bad))) {
      ...
      arr[g, 'beta', reg_idx] = NA; arr[g, 'bvar', reg_idx] = NA
      arr[g, 'pval', reg_idx] = NA; arr[g, 'qval', reg_idx] = NA
    }
...
cat('$CT (noLcb) total flagged:', n_flagged_total, '\n')
"
