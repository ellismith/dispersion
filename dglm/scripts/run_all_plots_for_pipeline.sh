...
for CT in $CELL_TYPES; do
  ...
  mv ${DGLM_BASE}/figures/bar_sig_by_region_celltype_age_mash_lfsr0.2.png \
     ${FIG_ROOT}/bar_charts/${CT}_celltype.png 2>/dev/null || true
  mv ${DGLM_BASE}/figures/bar_sig_by_region_subcluster_age_mash_lfsr0.2.png \
  ...
  $RSCRIPT $SCRIPTS/plot_upset_subtype_region.R \
    --ct $CT --pipeline baseline --mode subclusters --ymetric pct \
    --out ${FIG_ROOT}/upset_flexible/${CT}_subclusters_pct.png
  $RSCRIPT $SCRIPTS/plot_upset_subtype_region.R \
    --ct $CT --pipeline baseline --mode subclusters --ymetric count \
    --out ${FIG_ROOT}/upset_flexible/${CT}_subclusters_count.png
...
$RSCRIPT $SCRIPTS/fig3F_style_hist.R --all --lfsr_thresh 0.05 \
...
$RSCRIPT $SCRIPTS/plot_prop_increasing.R \
  --out ${FIG_ROOT}/prop_increasing/ALL_cell_types.png 2>/dev/null || true
