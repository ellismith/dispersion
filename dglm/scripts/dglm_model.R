...
if (!opt$dispersion %in% c('age', 'age_sex', 'age_ncells')) {
    stop("--dispersion must be 'age', 'age_sex', or 'age_ncells'")
}
...
disp.covs = if (opt$dispersion == 'age_sex') c('age', 'sex') else if (opt$dispersion == 'age_ncells') c('age', 'n_cells') else c('age')
...
for (region in region.levels) {
    ...
    # Low-point removal (the ONLY added filter): mask zero counts and
    # detached low values (below median - max(3*MAD, 2)) per gene.
    logCPM[counts <= 1] = NA   # counts 0 and 1 are non-measurements, not expression levels
    med = apply(logCPM, 1, median, na.rm=TRUE)
    madv = apply(logCPM, 1, mad, na.rm=TRUE)
    lowpt = sweep(logCPM, 1, med - pmax(3*madv, 2), '<')
    logCPM[lowpt & !is.na(lowpt)] = NA
    # gene testable only if detected (count>=2) in >=50% of animals -- the
    # original expression filter, with detection meaning a real measurement
    detected = rowSums(!is.na(logCPM))
    logCPM[detected < 0.5 * ncol(logCPM), ] = NA
    message('    low-point mask: ', sum(counts == 0), ' zeros + ',
            sum(lowpt, na.rm=TRUE), ' detached low points masked')
    ...
    results = t(parApply(clus, logCPM, 1, function(expr) {
        ...
        d = d[!is.na(d$e), ]   # low points masked upstream
        if (nrow(d) < 10) {
            return(c(
        ...
        disp.tab = try(coef(summary(res$dispersion.fit)), silent=TRUE)
...
saveRDS(
    list(
        ...
        run_info = list(
            ...
            dispersion_pval_source = 'summary(res$dispersion.fit), Gamma GLM t-test',
