...
option_list = list(
  ...
  make_option('--hide_masked', action='store_true', default=FALSE),
...
for (i in seq_len(nrow(selected))) {
  ...
  rawc = as.numeric(counts[s$ensembl_id, ])
  ev = as.numeric(logCPM[s$ensembl_id, ])
  em = ev; em[rawc <= 1] = NA
  cutv = median(em, na.rm=TRUE) - max(3*mad(em, na.rm=TRUE), 2)
  masked = (rawc <= 1) | (!is.na(ev) & ev < cutv)
...
if (opt$hide_masked) plot_df = plot_df[!plot_df$masked,]
p = ggplot(plot_df, aes(x=age, y=expr)) +
  geom_point(data=function(d) d[!d$masked,], aes(color=direction), size=2.8, alpha=0.80) +
  geom_point(data=function(d) d[d$masked,], color='grey55', shape=4, size=2.2, stroke=1.1) +
  geom_smooth(data=function(d) d[!d$masked,], 
    method='lm',
    se=TRUE,
    color='black',
    linewidth=0.9,
