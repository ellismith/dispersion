...
# clean out old (pre-filterByExpr-fix) results so nothing stale lingers
rm -rf ${FILTERED_DIR} ${STAGE_BASE} ${CHECKPOINTS}
mkdir -p ${STAGE_BASE} ${CHECKPOINTS} ${SPARSITY_DIR}

source /packages/apps/mamba/2.0.8/etc/profile.d/conda.sh
conda activate mashr_env

echo "=== 2. CPM filter + filterByExpr (corrected) -- pseudobulk itself reused, unchanged ==="
...
python3 /scratch/easmit31/dispersion/dglm/scripts/assess_condition_sparsity.py \
  --cell_type ${CT} --base_dir ${BASE} --out_dir ${SPARSITY_DIR} \
  --min_cells 300 --min_animals 30

conda activate mashr_env
mkdir -p ${BASE}/${CT}
ln -sf ${BASE}/${CT}_staged/dglm_checkpoints_cutoff0.5 ${BASE}/${CT}/dglm_checkpoints_cutoff0.5
Rscript /scratch/easmit31/dispersion/dglm/scripts/filter_conditions_for_mashr.R \
  ...
  --src_base ${BASE} --out_base ${FILTERED300_BASE}
...
echo "=== 6. QC filter (|beta|>10) + final mashr + master table ==="
...
echo "DONE: ${CT} min_cells_per_animal=${MINCELLS} (filterByExpr fix applied)"
